...
constexpr T byteswap(T v) noexcept {
    static_assert(!std::is_same_v<T, bool>, "byteswap not valid for bool");
    // used for bitcasts into the correct size unsigned integer type for byte
    // swapping
...
template <typename T>
concept EndianType = std::is_integral_v<T> || std::is_floating_point_v<T>;

template <EndianType T, std::endian Endian>
struct endian {
    using value_type = T;
...
template <EndianType T, std::endian E>
requires (sizeof(T) == 1)
struct endian<T, E> {
    using value_type = T;
    static constexpr std::endian endian_v = E;
    T repr_;

    constexpr T to() const { return repr_; }
    constexpr T native() const { return repr_; }
    constexpr T big() const { return repr_; }
    constexpr T little() const { return repr_; }
    constexpr T network() const {  repr_; }

    constexpr void from(T value) { repr_ = value; }
    constexpr void from_native(T value) { from(value); }
    constexpr void from_big(T value) { from(value); }
    constexpr void from_little(T value) { from(value); }
    constexpr void from_network(T value) { from(value); }

    endian() = default;

    constexpr explicit endian(T value) : repr_(value) {}
};
