...
class SimulationAddress {
   public:
    ...
    bool operator==(const SimulationAddress& other) const noexcept {
...
class PDUHeaderView {
   public:
    PDUHeaderView(BufferView buffer) : buffer_(buffer) {}

    ProtocolVersion protocol_version() const noexcept {
        return static_cast<ProtocolVersion>(
            be<std::uint8_t>(buffer_[0]).native());
    }
    std::uint8_t exercise_id() const noexcept {
        return be<std::uint8_t>(buffer_[1]).native();
    }
    PDUType pdu_type() const noexcept {
...
template <typename T>
struct pdu_type {
    static constexpr PDUType value = PDUType::Other;
};

template <>
struct pdu_type<EntityStatePDU> {
    static constexpr PDUType value = PDUType::EntityState;
};

template <>
struct pdu_type<ElectromagneticEmissionPDU> {
    static constexpr PDUType value = PDUType::ElectromagneticEmission;
};

template <>
struct pdu_type<TransmitterPDU> {
    static constexpr PDUType value = PDUType::Transmitter;
};
